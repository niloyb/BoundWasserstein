...
# ## Adding upper bound based on the mixture of distributions (Cor 3.2) ##
# coupled_crn_mala_mode1_sampler <- 
#   function(){
#     # Sample from Q (dimension globally defined)
#     SamplerP <- function(no_indep_samples){
#       output <- matrix(rnorm(dimension*no_indep_samples), nrow = no_indep_samples, ncol = dimension)
#       for (i in c(1:no_indep_samples)){output[i,] <- output[i,] + means_P[,1]}
#       return(output)
#     }
#     SamplerQ <- function(no_indep_samples){
#       output <- matrix(rnorm(dimension*no_indep_samples), nrow = no_indep_samples, ncol = dimension)
#       for (i in c(1:no_indep_samples)){output[i,] <- output[i,] + means_Q[,1]}
#       return(output)
#     }
#     # Log densities up to normalizing constant
#     LogPdfP <- function(x) {return(-0.5*sum((x-means_P[,1])^2))}
#     LogPdfQ <- function(x) {return(-0.5*sum((x-means_Q[,1])^2))}
#     # Gradient of log densities
#     GradLogPdfP <- function(x) {return(-(x-means_P[,1]))}
#     GradLogPdfQ <- function(x) {return(-(x-means_Q[,1]))}
#     
#     return(coupled_mala(SamplerP(1)[1,], SamplerQ(1)[1,], LogPdfP, LogPdfQ,
#                         GradLogPdfP, GradLogPdfQ, sigma_mh, chain_length))
#   }
# coupled_crn_mala_mode2_sampler <- 
#   function(){
#     # Sample from Q (dimension globally defined)
#     SamplerP <- function(no_indep_samples){
#       output <- matrix(rnorm(dimension*no_indep_samples), nrow = no_indep_samples, ncol = dimension)
#       for (i in c(1:no_indep_samples)){output[i,] <- output[i,] + means_P[,2]}
#       return(output)
#     }
#     SamplerQ <- function(no_indep_samples){
#       output <- matrix(rnorm(dimension*no_indep_samples), nrow = no_indep_samples, ncol = dimension)
#       for (i in c(1:no_indep_samples)){output[i,] <- output[i,] + means_Q[,2]}
#       return(output)
#     }
#     # Log densities up to normalizing constant
#     LogPdfP <- function(x) {return(-0.5*sum((x-means_P[,2])^2))}
#     LogPdfQ <- function(x) {return(-0.5*sum((x-means_Q[,2])^2))}
#     # Gradient of log densities
#     GradLogPdfP <- function(x) {return(-(x-means_P[,2]))}
#     GradLogPdfQ <- function(x) {return(-(x-means_Q[,2]))}
#     
#     return(coupled_mala(SamplerP(1)[1,], SamplerQ(1)[1,], LogPdfP, LogPdfQ,
#                         GradLogPdfP, GradLogPdfQ, sigma_mh, chain_length, 
#                         reflect_threshold = 0))
#   }
# 
# coupled_crn_mala_mode1_trajectory <- 
#   wp_ub_estimate(coupled_crn_mala_mode1_sampler, no_chains=1000, p=p, 
#                  metric=metric_l2, parallel=TRUE)
# coupled_crn_mala_mode2_trajectory <- 
#   wp_ub_estimate(coupled_crn_mala_mode2_sampler, no_chains=1000, p=p, 
#                  metric=metric_l2, parallel=TRUE)
# mixture_crn_metric <- 
#   (0.5*coupled_crn_mala_mode1_trajectory$wp_power_p_ub_mean_tracjectory +
#      0.5*coupled_crn_mala_mode2_trajectory$wp_power_p_ub_mean_tracjectory)^(1/p)
# crn_reflection_df <- 
#   rbind(crn_reflection_df,
#         data.frame(t=c(1:chain_length), type='mixture_crn', 
#                    trajectory=5, metric=mixture_crn_metric))
# # save(crn_reflection_df, file="stylized_example_ula_mala/crn_reflection_df.RData")
# # load("stylized_example_ula_mala/crn_reflection_df.RData")
# 
# plot.labels <- 
#   unname(TeX(c('$CUB_1$ (CRN)','$CUB_1$ (Reflect)','True $W_1$', 
#                'Indep. Coupling', 'Cor. $3.2$ CRN')))
# crn_reflection_plot <- 
#   ggplot(crn_reflection_df %>% filter(type!='mixture_crn'), 
#          aes(x=t, y=metric, color=interaction(type, trajectory), 
#              linetype=interaction(type, trajectory))) + 
#   geom_line(size=1) +
#   xlab(TeX('iteration $t$')) + ylab(TeX('$W_1$ upper bounds')) +
#   scale_colour_manual(name=TeX(''), labels=plot.labels,
#                       values = c('lightgrey', 'black', 'black', 'black', 'grey')) +
#   scale_linetype_manual(name=TeX(''), labels=plot.labels,
#                         values = c('solid', 'solid', 'dotdash', 'dotted', 'dotted')) +
#   #scale_linetype_discrete(guide = "none") +
#   #scale_linetype_manual(breaks = c("coupling", "coupling_averaged", "true_w1", "indep_coupling"))+
#   # scale_x_continuous(breaks=seq(0,1e3,100), limits = c(0,200)) +
#   scale_y_continuous(breaks=seq(0,12,1), limits = c(0,2.5)) +
...
# crn_reflection_plot
# #ggsave(filename = "/Users/niloybiswas/Dropbox/Apps/Overleaf/couplings/writeup/images/implementation/crn_reflection_plot1a.pdf", plot = crn_reflection_plot, width = 4, height = 5)
