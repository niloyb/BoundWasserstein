...
# ### Sinkhorn with coupled samples
# q_samples_coupled <- matrix(NA, nrow = dimension, ncol = 0)
# pi_samples_coupled <- matrix(NA, nrow = dimension, ncol = 0)
# for (c in 1:no_chains){
#   samples <- coupled_chain_sampler_mala()
#   q_samples_coupled <- cbind(q_samples_coupled, t(samples$Q))
#   pi_samples_coupled <- cbind(pi_samples_coupled, t(samples$P))
#   print(c)
# }
# 
# cost_mat_coupled <- matrix(NA, n_samples, n_samples)
# for (i in 1:(n_samples)){
#   for (j in 1:(n_samples)){
#     cost_mat_coupled[i,j] <- metric_l2(q_samples_coupled[,i],pi_samples_coupled[,j])^2
#   }
#   print(i)
# }
# 
# lambda_grid <- seq(0.25,10,0.25)
# nrepeats <- 1
# sinkhorn_distances_coupled <- rep(NA, length(lambda_grid))
# time_taken_sinkhorn_coupled <- rep(NA, length(lambda_grid))
# for (i in 1:length(lambda_grid)){
#   time_taken_repeats <- rep(NA, nrepeats)
#   sinkhorn_repeats <- rep(NA, nrepeats)
#   for(j in 1:nrepeats){
#     start.time <- proc.time()[[3]]
#     output <- Sinkhorn(matrix(1/n_samples,n_samples,1), matrix(1/n_samples,n_samples,1), 
#                        cost_mat_coupled, lambda=lambda_grid[i])
#     end.time <- proc.time()[[3]]
#     time_taken_repeats[j] <- round(end.time - start.time,2)
#     sinkhorn_repeats[j] <- (output$Distance)^0.5
#   }
#   time_taken_sinkhorn_coupled[i] <- mean(time_taken_repeats)
#   sinkhorn_distances_coupled[i] <- mean(sinkhorn_repeats)
#   print(i)
# }
# 
# sinkhorn_df <-
#   rbind(sinkhorn_df,
#         data.frame(lambda=lambda_grid, metric=sinkhorn_distances_coupled,
#                    time=time_taken_sinkhorn_coupled, algo='coupled_sinkhorn'))
